 ...
 * @author davidchenlo
 */
public class VentaProductoFormModelo {

    private final MySQL mySQL1;
    private MySQL mySQL2;

    public VentaProductoFormModelo() {
    ...
    public int calcularCostoTotal(int cantidad_producto, int id_producto, int id_tamaño) {
        int precio_producto = 0;
        final String sql = "select precio_producto from tamaño_producto where id_producto=? and id_tamaño=? and valid=?";
        try {
            this.setMySQL2(new MySQL());
            this.getMySQL2().connect();
            this.getMySQL2().setPreparedStatement(this.getMySQL2().getConnection().prepareStatement(sql));
            this.getMySQL2().getPreparedStatement().setInt(1, id_producto);
            this.getMySQL2().getPreparedStatement().setInt(2, id_tamaño);
            this.getMySQL2().getPreparedStatement().setBoolean(3, true);
            this.getMySQL2().setResultSet(this.getMySQL2().getPreparedStatement().executeQuery());
            if (this.getMySQL2().getResultSet().next()) {
                precio_producto = this.getMySQL2().getResultSet().getInt(1);
            }
            this.getMySQL2().getResultSet().close();
            this.getMySQL2().getPreparedStatement().close();
            this.getMySQL2().disconnect();
        ...
        return (cantidad_producto*precio_producto);
    }
